    ...
    public class StagePrepCareTerminationRepository : IStagePrepCareTerminationRepository
    {
        ...
        public async Task SyncStage(List<StagePrepCareTermination> extracts, Guid manifestId)
        {
            try
            {
                ...
                var notification = new ExtractsReceivedEvent { TotalExtractsStaged = extracts.Count, ManifestId = manifestId, SiteCode = extracts.First().SiteCode, ExtractName = "PrepCareTerminations" };
            ...
            try
            {
                ...
                var query = $@"
                            ...
                            WHERE EXISTS (
                                SELECT 1
                                FROM (
                                    SELECT PatientPK, SiteCode, PrepNumber
                                    FROM {_stageName} WITH (NOLOCK)
                                    WHERE 
                                        ManifestId = @manifestId 
                                        AND LiveStage = @livestage
                                    GROUP BY PatientPK, SiteCode, PrepNumber
                                ) s
                ...
                var existingRecordsSet = new HashSet<(int PatientPK, int SiteCode, string PatientMnchID)>(existingRecords.Select(x => (x.PatientPk, x.SiteCode, x.PrepNumber)));
                ...
                if (existingRecordsSet.Any())
                {
                    ...
                    uniqueStageExtracts = stagePrepCt
                        .Where(x => !existingRecordsSet.Contains((x.PatientPk, x.SiteCode, x.PrepNumber)) && x.ManifestId == manifestId)
                        .ToList();
        ...
        private async Task InsertNewDataFromStaging(List<StagePrepCareTermination> uniqueStageExtracts)
        {
            try
            {
                ...
                {
                    var key = $"{extract.PatientPk}_{extract.SiteCode}_{extract.PrepNumber}";
        ...
        private async Task UpdateCentralDataWithStagingData(List<StagePrepCareTermination> stageDrug, IEnumerable<PrepCareTermination> existingRecords)
        {
            try
            {
                ...
                var stageDictionary = stageDrug
                         .GroupBy(x => new { x.PatientPk, x.SiteCode, x.PrepNumber })
                         .ToDictionary(
                             g => g.Key,
                             g => g.FirstOrDefault()
                         );
                ...
                foreach (var existingExtract in existingRecords)
                {
                    if (stageDictionary.TryGetValue(
                        new { existingExtract.PatientPk, existingExtract.SiteCode, existingExtract.PrepNumber },
