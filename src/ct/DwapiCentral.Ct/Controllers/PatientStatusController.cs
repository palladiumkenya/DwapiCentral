...
namespace DwapiCentral.Ct.Controllers
{
    public class PatientStatusController : Controller
    {
        ...
        public async Task<IActionResult> PostBatchNew([FromBody] List<PatientStatusProfile> patientProfile)
        {
            if (null != patientProfile && patientProfile.Any())
            {
                try
                {
                    BackgroundJob.Enqueue(() => SaveDiffData(new MergeDifferentialStatusCommand(patientProfile)));
            ...
            return BadRequest($"The expected '{new PatientStatusProfile().GetType().Name}' is null");
        }

        public async Task SaveDiffData(MergeDifferentialStatusCommand saveDiffCommand)
        {
