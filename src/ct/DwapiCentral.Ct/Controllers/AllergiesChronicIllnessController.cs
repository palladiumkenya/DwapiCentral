...
namespace DwapiCentral.Ct.Controllers
{
    public class AllergiesChronicIllnessController : Controller
    {
        ...
        public async Task<IActionResult> PostBatchDiff([FromBody] List<AllergiesChronicIllnessProfile> patientProfile)
        {
            if (null != patientProfile && patientProfile.Any())
            {
                try
                {
                    BackgroundJob.Enqueue(() => SaveDiffData(new MergeDifferentialAllergiesCommand(patientProfile)));
            ...
            return BadRequest($"The expected '{new AllergiesChronicIllnessProfile().GetType().Name}' is null");
        }


        public async Task SaveDiffData(MergeDifferentialAllergiesCommand saveDiffCommand)
        {
