...
namespace DwapiCentral.Ct.Controllers
{
    public class IITRiskScoreController : Controller
    {
        ...
        public async Task<IActionResult> PostBatchNew([FromBody] IITRiskScoreSourceBag sourceBag)
        {
            ...
            if (null != sourceBag && sourceBag.Extracts.Any())
            {
                ...
                try
                {
                    ...
                    var notification = new ExtractsReceivedEvent { TotalExtractsStaged = sourceBag.Extracts.Count, ManifestId = sourceBag.ManifestId, SiteCode = sourceBag.Extracts.First().SiteCode, ExtractName = "IITRiskScoresExtract" };
        ...
        public async Task<IActionResult> PostBatchNew([FromBody] List<IITRiskScoresProfile> patientProfile)
        {
            if (null != patientProfile && patientProfile.Any())
            {
                try
                {
                    BackgroundJob.Enqueue(() => SaveDiffData(new MergeDifferentialIITRiskScoreCommand(patientProfile)));
            ...
            return BadRequest($"The expected '{new IITRiskScoresProfile().GetType().Name}' is null");
        }


        public async Task SaveDiffData(MergeDifferentialIITRiskScoreCommand saveDiffCommand)
        {
