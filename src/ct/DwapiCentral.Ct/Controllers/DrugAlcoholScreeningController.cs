...
namespace DwapiCentral.Ct.Controllers
{
    public class DrugAlcoholScreeningController : Controller
    {
        ...
        public async Task<IActionResult> PostBatch([FromBody] DrugAlcoholScreeningSourceBag sourceBag)
        {
            if (null != sourceBag && sourceBag.Extracts.Any())
            {
                ...
                try
                {
                    ...
                    var notification = new ExtractsReceivedEvent { TotalExtractsStaged = sourceBag.Extracts.Count, ManifestId = sourceBag.ManifestId, SiteCode = sourceBag.Extracts.First().SiteCode, ExtractName = "DrugAlcoholScreeningExtract" };
        ...
        public async Task<IActionResult> PostBatchNew([FromBody] List<DrugAlcoholScreeningProfile> patientProfile)
        {
            if (null != patientProfile && patientProfile.Any())
            {
                try
                {
                    BackgroundJob.Enqueue(() => SaveDiffData(new MergeDifferentialDrugCommand(patientProfile)));
            ...
            return BadRequest($"The expected '{new DrugAlcoholScreeningProfile().GetType().Name}' is null");
        }


        public async Task SaveDiffData(MergeDifferentialDrugCommand saveDiffCommand)
        {
