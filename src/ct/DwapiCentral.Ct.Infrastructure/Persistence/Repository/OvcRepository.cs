    ...
    public class OvcRepository : IOvcRepository
    {
        ...
        public async Task UpdateExtract(List<OvcExtract> patientExtract)
        {
            try
            {
                ...
                var notification = new ExtractsReceivedEvent { TotalExtractsProcessed = patientExtract.Count, ManifestId = manifestId, SiteCode = patientExtract.First().SiteCode, ExtractName = "OvcExtract", UploadMode = UploadMode.DifferentialLoad };
                await _mediator.Publish(notification);
