...
namespace DwapiCentral.Ct.Infrastructure.Persistence.Repository.Stage
{
    public class StageDepressionScreeningExtractRepository :IStageDepressionScreeningExtractRepository
    {
        ...
        private async Task MergeExtracts(Guid manifestId, List<StageDepressionScreeningExtract> stageDepression)
        {
            ...
            try
            {
                ...
                if (existingRecordsSet.Any())
                {
                    ...
                    await UpdateCentralDataWithStagingData(stageDepression, existingRecords,manifestId);
                }
                else
                {
                    uniqueStageExtracts = stageDepression;
        ...
        private async Task InsertNewDataFromStaging(List<StageDepressionScreeningExtract> uniqueStageExtracts,Guid manifestId)
        {
            try
            {
                var sortedExtracts = uniqueStageExtracts.OrderByDescending(e => e.Date_Created).ToList();
                var latestRecordsDict = new Dictionary<string, StageDepressionScreeningExtract>();
                ...
                var mappedExtracts = _mapper.Map<List<DepressionScreeningExtract>>(filteredExtracts);
                _context.Database.GetDbConnection().BulkInsert(mappedExtracts);
            }
            catch (Exception ex)
            {
                Log.Error(ex);
                var notification = new OnErrorEvent { ExtractName = "DepressionScreeningExtract", ManifestId = manifestId, SiteCode = uniqueStageExtracts.First().SiteCode, message = ex.Message };
                await _mediator.Publish(notification);
                throw;
            }
        }

        private async Task UpdateCentralDataWithStagingData(List<StageDepressionScreeningExtract> stageDepression, IEnumerable<DepressionScreeningExtract> existingRecords,Guid manifestId)
        {
                try
                {
                    ...
                    for (var retry = 0; retry < maxRetries; retry++)
                    {
                        ...
                        catch (SqlException ex)
                        {
                            ...
                            {
                                Log.Error(ex);
                                var notification = new OnErrorEvent { ExtractName = "DepressionScreeningExtract", ManifestId = manifestId, SiteCode = existingRecords.First().SiteCode, message = ex.Message };
                                await _mediator.Publish(notification);
                                throw;
                            }
                        }
