...
namespace DwapiCentral.Ct.Infrastructure.Persistence.Repository.Stage
{
    public class StageAllergiesChronicIllnessExtractRepository : IStageAllergiesChronicIllnessExtractRepository
    {
        ...
        private async Task MergeExtracts(Guid manifestId, List<StageAllergiesChronicIllnessExtract> stageAllergies)
        {
            ...
            try
            {
                ...
                if (existingRecordsSet.Any())
                {
                    ...
                    await UpdateCentralDataWithStagingData(stageAllergies, existingRecords,manifestId);                   
        ...
        private async Task InsertNewDataFromStaging(List<StageAllergiesChronicIllnessExtract> uniqueStageExtracts, Guid manifestId)
        {
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                var notification = new OnErrorEvent { ExtractName = "AllergiesChronicIllnessExtract", ManifestId = manifestId, SiteCode = uniqueStageExtracts.First().SiteCode, message = ex.Message };
                await _mediator.Publish(notification);
        ...
        private async Task UpdateCentralDataWithStagingData(List<StageAllergiesChronicIllnessExtract> stageAllergies, IEnumerable<AllergiesChronicIllnessExtract> existingRecords, Guid manifestId)
        {
                try
                {
                    ...
                    for (var retry = 0; retry < maxRetries; retry++)
                    {
                        ...
                        catch (SqlException ex)
                        {
                            ...
                            {
                                Log.Error(ex);
                                var notification = new OnErrorEvent { ExtractName = "AllergiesChronicIllnessExtract", ManifestId = manifestId, SiteCode = existingRecords.First().SiteCode, message = ex.Message };
                                await _mediator.Publish(notification);
                                throw;
                            }
                        }
