...
namespace DwapiCentral.Ct.Infrastructure.Persistence.Repository.Stage
{
    public class StageDrugAlcoholScreeningExtractRepository :IStageDrugAlcoholScreeningExtractRepository
    {
        ...
        private async Task MergeExtracts(Guid manifestId, List<StageDrugAlcoholScreeningExtract> stageDrug)
        {
            ...
            try
            {
                ...
                if (existingRecordsSet.Any())
                {
                    ...
                    await UpdateCentralDataWithStagingData(stageDrug, existingRecords,manifestId);
        ...
        private async Task InsertNewDataFromStaging(List<StageDrugAlcoholScreeningExtract> uniqueStageExtracts,Guid manifestId)
        {
            try
            {
            ...
            {
                Log.Error(ex);
                var notification = new OnErrorEvent { ExtractName = "DrugAlcoholScreeningExtract", ManifestId = manifestId, SiteCode = uniqueStageExtracts.First().SiteCode, message = ex.Message };
                await _mediator.Publish(notification);
                throw;
            }
        }

        private async Task UpdateCentralDataWithStagingData(List<StageDrugAlcoholScreeningExtract> stageDrug, IEnumerable<DrugAlcoholScreeningExtract> existingRecords,Guid manifestId)
        {
                ...
                try
                {
                    ...
                    for (var retry = 0; retry < maxRetries; retry++)
                    {
                        ...
                        catch (SqlException ex)
                        {
                            ...
                            {
                                Log.Error(ex);
                                var notification = new OnErrorEvent { ExtractName = "DrugAlcoholScreeningExtract", ManifestId = manifestId, SiteCode = existingRecords.First().SiteCode, message = ex.Message };
                                await _mediator.Publish(notification);
                                throw;
                            }
                        }
