    ...
    public class StageDrugAlcoholScreeningExtractRepository :IStageDrugAlcoholScreeningExtractRepository
    {
        ...
        public async Task SyncStage(List<StageDrugAlcoholScreeningExtract> extracts, Guid manifestId)
        {
            try
            {
                ...
                var notification = new ExtractsReceivedEvent { TotalExtractsCount = extracts.Count, SiteCode = extracts.First().SiteCode, ExtractName = "DrugAlcoholScreeningExtracts" };
        ...
        private async Task MergeExtracts(Guid manifestId, List<StageDrugAlcoholScreeningExtract> stageDrug)
        {
            ...
            try
            {
                ...
                var existingRecords = await connection.QueryAsync<DrugAlcoholScreeningExtract>("SELECT * FROM DrugAlcoholScreeningExtracts WHERE PatientPk IN @PatientPKs AND SiteCode IN @SiteCodes AND VisitID IN @VisitIDs AND VisitDate IN @VisitDates ",
                    new
                    {
                ...
                if (existingRecordsSet.Any())
                {
                    ...
                    {
                        var stageExtract = stageDrug.FirstOrDefault(x =>
                ...
                var drugExtracts = _mapper.Map<List<DrugAlcoholScreeningExtract>>(uniqueStageExtracts);
                _context.Database.GetDbConnection().BulkInsert(drugExtracts);
