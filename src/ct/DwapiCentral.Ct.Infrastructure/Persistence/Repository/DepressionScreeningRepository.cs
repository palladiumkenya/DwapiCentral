    ...
    public class DepressionScreeningRepository : IDepressionScreeningRepository
    {
        ...
        public async Task UpdateExtract(List<DepressionScreeningExtract> patientExtract)
        {
            try
            {
                ...
                var notification = new ExtractsReceivedEvent { TotalExtractsProcessed = patientExtract.Count, ManifestId = manifestId, SiteCode = patientExtract.First().SiteCode, ExtractName = "DepressionScreeningExtract", UploadMode = UploadMode.DifferentialLoad };
                await _mediator.Publish(notification);
