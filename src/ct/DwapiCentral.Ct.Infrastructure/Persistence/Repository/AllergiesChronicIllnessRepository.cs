    ...
    public class AllergiesChronicIllnessRepository : IAllergiesChronicIllnessRepository
    {
        ...
        public async Task UpdateExtract(List<AllergiesChronicIllnessExtract> patientExtract)
        {
            try
            {
                ...
                var notification = new ExtractsReceivedEvent { TotalExtractsProcessed = patientExtract.Count, ManifestId = manifestId, SiteCode = patientExtract.First().SiteCode, ExtractName = "AllergiesChronicIllnessExtract", UploadMode = UploadMode.DifferentialLoad };
                await _mediator.Publish(notification);
